...
async def start_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game_active = True
    pot = count * ENTRY_FEE
    # PRIZE RULES: 10% 1-line, 15% 2-line, 20% 3-line, 25% Full House (Total 70% return)
    rules = (f"🚀 **BINGO START!**\n\n"
             f"💰 **Total Pot: {pot} ETB**\n"
             ...
             f"🚫 Chat Locked! Watch the screen.")
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Admin is checking your payment. Please wait!")
...
async def admin_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data[0] == "app":
        ...
        await context.bot.send_message(chat_id=user_id, text="✅ Verified! Here is your card:", reply_markup=kb)
        await query.edit_message_reply_markup(reply_markup=None)
    
    elif data[0] == "win":
        global game_active, called_numbers
        username = data[2]
        await context.bot.send_message(chat_id=GROUP_CHAT_ID, text=f"🎊 **WINNER: @{username}**\n\n⚠️ Game Reset. All previous payments expired. Pay 10 ETB for the next game!")
        game_active = False
        called_numbers = []
        conn = sqlite3.connect("bingo.db")
        ...
        conn.commit()
        conn.close()
        for job in context.job_queue.get_jobs_by_name("caller_job"): job.schedule_removal()
        await query.edit_message_reply_markup(reply_markup=None)

async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 Welcome! Send a screenshot of your 10 ETB payment to join the next game.")
