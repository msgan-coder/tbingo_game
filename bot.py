...
def keep_alive():
    """Pings the server every 10 minutes to prevent sleeping"""
    while True:
        try:
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data[0] == "win":
        ...
        await context.bot.send_message(chat_id=GROUP_CHAT_ID, text=f"❌ @{username} card rejected. Resuming...")
        await query.edit_message_text(text=f"❌ Rejected: @{username}")
